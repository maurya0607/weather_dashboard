...
export default function HistoricalWeather({ location }) {
  ...
  const chartData =
    data?.daily?.time?.map((t, i) => ({
      ...
      "Mean Temp":
        data.daily.temperature_2m_mean?.[i] != null
          ? +data.daily.temperature_2m_mean[i].toFixed(1)
          : null,
      ...
      "Max Wind":
        data.daily.wind_speed_10m_max?.[i] != null
          ? +data.daily.wind_speed_10m_max[i].toFixed(1)
          : null,

      "Wind Dir": data.daily.wind_direction_10m_dominant?.[i] ?? null,

      PM10:
        data.daily.pm10?.[i] != null
          ? +data.daily.pm10[i].toFixed(1)
          : null,

      PM2_5:
        data.daily.pm2_5?.[i] != null
          ? +data.daily.pm2_5[i].toFixed(1)
          : null,

      CO:
        data.daily.carbon_monoxide?.[i] != null
          ? +data.daily.carbon_monoxide[i].toFixed(1)
          : null,

      NO2:
        data.daily.nitrogen_dioxide?.[i] != null
          ? +data.daily.nitrogen_dioxide[i].toFixed(1)
          : null,

      SO2:
        data.daily.sulphur_dioxide?.[i] != null
          ? +data.daily.sulphur_dioxide[i].toFixed(1)
          : null,
  ...
  const summary = data?.daily
    ? (() => {
        ...
        return {
          avgMax: avg(data.daily.temperature_2m_max || []),
          avgMin: avg(data.daily.temperature_2m_min || []),
          totalPrecip: (
            data.daily.precipitation_sum?.reduce((a, b) => a + b, 0) || 0
          ).toFixed(1),
          maxWind:
            data.daily.wind_speed_10m_max?.length
              ? Math.max(...data.daily.wind_speed_10m_max).toFixed(0)
              : "—",
        };
      })()
    : null;
  ...
  const TILES = [
    {
      ...
      key: "avgMax",
      unit: "°C",
      icon: <WiThermometer size={22} color="#fc8181" />,
    ...
    },
    {
      ...
      key: "avgMin",
      unit: "°C",
      icon: <WiThermometer size={22} color="#63b3ed" />,
    ...
    },
    {
      ...
      key: "totalPrecip",
      unit: "mm",
      icon: <WiRain size={22} color="#76e4f7" />,
    ...
    },
    {
      ...
      key: "maxWind",
      unit: "km/h",
      icon: <WiStrongWind size={22} color="#68d391" />,
    ...
    <>
      ...
      <div className="hist-page">
        ...
        <p className="hist-subtitle">
          Explore past weather for {location.name} — up to 2 years
        </p>
        ...
        {!loading && !error && !data && (
          <div className="empty-state">
            <div className="empty-icon">
              <WiThermometer size={56} color="#4a5a78" />
            </div>
            <p>
              Select a date range and click <strong>Fetch Data</strong>.
            </p>
          </div>
        )}
          ...
          <>
              ...
              <div className="summary-row">
                {TILES.map((t) => (
                  <div className="summary-tile" key={t.key}>
                    <div className="tile-icon-wrap" style={{ background: t.bg }}>
                      {t.icon}
                    </div>
                    <div>
            ...
            <div className="charts-stack">
              ...
              <ChartCard
                ...
                icon={<WiThermometer size={18} color="#fc8181" />}
              >
                <AreaChart
                  data={chartData}
              ...
              {/* Sun Cycle */}
              <ChartCard
                title="Sun Cycle — Sunrise & Sunset (IST)"
                icon={<WiSunrise size={18} color="#f6c90e" />}
              >
                <LineChart
                  data={chartData}
                  margin={{ top: 5, right: 20, left: -10, bottom: 0 }}
              ...
              <ChartCard
                ...
                icon={<WiRain size={18} color="#76e4f7" />}
              >
                <BarChart
                  data={chartData}
                  margin={{ top: 5, right: 20, left: -10, bottom: 0 }}
              ...
              <ChartCard
                ...
                icon={<WiStrongWind size={18} color="#68d391" />}
              >
                <AreaChart
                  data={chartData}
                  margin={{ top: 5, right: 20, left: -10, bottom: 0 }}
                >
                  <CartesianGrid {...gridProps} />
                  <XAxis dataKey="date" {...axisProps} />
                  <YAxis yAxisId="left" {...axisProps} unit=" km/h" />
                  <YAxis
                    yAxisId="right"
                    orientation="right"
                    {...axisProps}
              ...
              {/* PM10 & PM2.5 */}
              <ChartCard
                title="Air Quality — PM10 & PM2.5 (µg/m³)"
                icon={<MdAir size={16} color="#76e4f7" />}
              >
                <LineChart
                  data={chartData}
                  margin={{ top: 5, right: 20, left: -10, bottom: 0 }}
              ...
              {/* CO, NO2, SO2 */}
              <ChartCard
                title="Pollutants — CO, NO₂ & SO₂ (µg/m³)"
                icon={<MdAir size={16} color="#b794f4" />}
              >
                <LineChart
                  data={chartData}
                  margin={{ top: 5, right: 20, left: -10, bottom: 0 }}
